...
###############################
    ...
    cdef start(self):
        ...
        cdef double time, delta_t, ke, ke_nm, se, beta, temperature, latticespacing
	...
	# MATT-Introduce X-COM variable, number of molecules Nmol
        ...
        # gotta find the lattice spacing...

        self.atomtocm(x,v,g,m,xcm,vcm,gcm,mcm,bdcm,Nmol)

        print xcm 

        latticespacing = sqrt((xcm[nbeadspmol][0] - xcm[0][0])**2 + (xcm[nbeadspmol][1] - xcm[0][1])**2 + (xcm[nbeadspmol][2] - xcm[0][2])**2 )

        print latticespacing        

        latticespacing = (round(latticespacing*1000.0))/100.0
...
##NEW DENSITY
        ...
        filename = "acceptanceratio-steps"+str(nsteps)+"nmolecules"+str(Nmol)+"nbeads"+str(P)  # specify things
        filestream = open(filename, "a")                                                                                      # a for append
        filestream.write(str(latticespacing)+"   "+str(int(Temp))+"    "+str(acceptratio)+"\n") 
        filestream.close()
